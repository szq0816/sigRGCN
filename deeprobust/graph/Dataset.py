...
from preprocessH5 import load_h5
...
class Dataset():
    ...
    def __init__(self, root, name, setting='nettack', seed=None, require_mask=False):
        ...
        self.data_filename = self.data_folder + '.h5'
        ...
        if setting == 'prognn':
            ...
            val_data = 0.2*len(index)
            test_data = 0.3*len(index)

            self.idx_train = index[0:int(np.ceil(train_data))]
            for i in self.idx_train:
                index.remove(i)
            self.idx_val = list(index[0:int(np.ceil(val_data))])
            ...
            self.idx_train = index[0:int(np.ceil(train_data))]
            index = np.delete(index, self.idx_train)
            self.idx_val = index[0:int(np.ceil(val_data))]
            index = np.delete(index, self.idx_val)
            self.idx_test = index[0:int(np.ceil(test_data))]
    ...
    def get_prognn_splits(self):
        ...
        json_file = os.getcwd()+self.root
        json_file = osp.join(json_file,
    ...
    def download_zip(self, name):
        url = os.getcwd()+self.root+'{}.zip'. format(name)
        try:
    ...
    def load_zip(self):
        data_filename = os.getcwd()+self.data_folder + '.zip'
        name = self.name
        if not osp.exists(data_filename):
            self.download_zip(name)
        with zipfile.ZipFile(data_filename, 'r') as zip_ref:
            zip_ref.extractall(os.getcwd()+self.root)

        feature_path = osp.join(os.getcwd()+self.data_folder, '{0}.feature'.format(name))
        label_path = osp.join(os.getcwd()+self.data_folder, '{0}.label'.format(name))
        graph_path = osp.join(os.getcwd()+self.data_folder, '{0}.edge'.format(name))
    ...
    def load_pubmed(self):
        ...
        for i in range(len(names)):
            ...
            data_filename = os.getcwd()+self.root + '/pubmed/'
            data_filename = osp.join(data_filename, name)
        ...
        file = os.getcwd()+self.root + '/pubmed/'
        test_idx_reorder = parse_index_file(osp.join(file, test_idx_file))
    ...
    def get_adj(self):
        adata,adj, features, labels = self.load_npz(self.data_filename)
        adj = adj + adj.T
        adj = adj.tolil()
        adj[adj > 1] = 1
    ...
    def load_npz(self, file_name, is_sparse=True):
        ...
        return adata,adj_gene, features, labels_gene
