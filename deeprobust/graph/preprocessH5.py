...
def prepro(filename,sparsify=False, skip_exprs=False):
    ...
    np.savetxt(fname="data.csv", X=X, fmt="%.3f", delimiter=",")
    np.savetxt(fname="label.csv", X=cell_label, fmt="%d",delimiter=",")
...
def load_h5(filename,n_gene=None, sparsify = False, skip_exprs = False):
    ...
    return adata,X, Y, count_X, size_factor, cluster_number
